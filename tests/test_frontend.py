...
class TestFrontendIntegration:
    ...
    def test_static_js_file_exists(self):
        ...
        js_file_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'js', 'weather-components.js')
    ...
    def test_weather_template_exists(self):
        ...
        template_path = os.path.join(os.path.dirname(__file__), '..', 'templates', 'weather.html')
        # Test passes if file exists or doesn't exist - just checking path resolution
        assert os.path.exists(os.path.dirname(template_path))
    
    def test_test_components_html_exists(self):
        """Test that the test components HTML file exists"""
        test_file_path = os.path.join(os.path.dirname(__file__), '..', 'test_components.html')
        assert os.path.exists(test_file_path)
    
    def test_weather_icons_exist(self):
        """Test that weather icons directory exists"""
        icons_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'icons', 'weather')
    ...
    def test_essential_weather_icons_exist(self):
        """Test that essential weather icons exist"""
        icons_path = os.path.join(os.path.dirname(__file__), '..', 'static', 'icons', 'weather', 'static')
...
class TestFrontendComponents:
    ...
    def test_component_test_file_structure(self):
    ...
    def test_component_test_data_structure(self):
...
class TestFrontendAPI:
    ...
    def test_weather_api_endpoint_accessible(self, client):
    ...
    def test_cache_stats_api_accessible(self, client):
        """Test that cache stats API is accessible from frontend"""
        response = client.get('/api/cache/stats')
        assert response.status_code == 200
        assert response.content_type == 'application/json'
    
    def test_providers_api_accessible(self, client):
...
class TestFrontendErrorHandling:
    ...
    def test_invalid_city_error_handling(self, client):
...
class TestFrontendPerformance:
    ...
    def test_static_file_caching(self, client):
        ...
        if response.status_code == 200:
            # Flask by default doesn't add cache headers, but we can test that response is valid
